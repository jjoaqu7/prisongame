...
**Agreed subsequent purpose:** another inmate occupies the neighboring cell, and the player kills him to close this prototype's progression loop. See the encounter scope below. The current build still contains only furnishings. **Open:** the inmate's identity, the player's motivation, encounter mechanics, consequences of killing/blackmail, and confirmation of slot/stack interpretation.
...
### Neighboring-cell inmate encounter - agreed purpose, mechanics open
...
**Proposed first implementation:** one clearly labelled, deliberate interaction for the encounter, a simple death response, a saved alive/dead state and an explicit completion outcome. This would test the complete access-and-objective chain before investing in a general combat system. The interaction must be distinct from ordinary talk/open-door input so it cannot happen through an accidental conversation press. These mechanics are a recommendation, not yet selected.

**Open decisions:** a staged interaction versus combat with resistance; how the objective/motivation is introduced; the inmate's identity; weapon requirements, if any; presentation; and what follows the killing, including any witness/guard response, punishment or reward. Selecting this encounter does not select all of those systems or imply that killing is consequence-free in the final game. No combat, death or encounter completion is implemented in the current build.
