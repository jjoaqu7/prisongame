...
- For research, follow the `game-research` skill at `.agents/skills/game-research/SKILL.md` (open Agent Skills format; Codex loads it natively, Claude Code through `.claude/skills/game-research/`). Output goes to `docs/research/<topic>/`: a `report.md` entry point plus a `notes/` folder. Research findings are references for decisions, not decisions.
