...
- Research goes in `docs/research/<topic>/` and follows `docs/research/README.md`: a `report.md` entry point plus a `notes/` folder. Research findings are references for decisions, not decisions. In Claude Code, use the `game-researcher` and `research-writer` agents in `.claude/agents/`.
